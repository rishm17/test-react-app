...
import React, {
  ...
  const ChakraCarousel = ({ children, gap, mb_constraint_value, divider }) => {
    ...
    useEffect(() => {
      if (isBetweenBaseAndMd) {
        setItemWidth(sliderWidth/divider-gap);
        setMultiplier(0.65);
        setConstraint(mb_constraint_value);
      }
      if (isBetweenMdAndXl) {
  ...
  const Slider = ({
    ...
    return (
      <>
        <Flex w={{ base: "100%", md: `calc(100% + ${gap}px)` }}>
          <Button
